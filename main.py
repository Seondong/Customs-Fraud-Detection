...
import datetime
from query_strategies import badge, bATE, random, DATE, diversity, uncertainty, hybrid, xgb, xgb_lr, ssl_ae, tabnet, semiDATE
import numpy as np
import torch
...
if __name__ == '__main__':
    ...
    parser.add_argument('--sampling', type=str, default = 'bATE', choices=['random', 'xgb', 'xgb_lr', 'DATE', 'diversity', 'badge', 'bATE', 'hybrid', 'tabnet', 'ssl_ae', 'noupDATE', 'randomupDATE', 'adahybrid', 'semiDATE'], help='Sampling strategy')
    ...
    for i in range(numWeeks):
        ...
        def initialize_sampler(samp):
            ...
            if samp == 'semiDATE': # check
                sampler = semiDATE.semiDATESampling(data, args)
            return sampler
