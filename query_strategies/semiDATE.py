...
class semiDATESampling(Strategy):
    """ semiDATE strategy: Using DATE architecture for semi-supervised anomaly detection """
    
    def __init__(self, data, args):
        super(semiDATESampling,self).__init__(data, args)
    ...
    def prepare_semiDATE_input(self):
        """ Prepare input for Dual-Attentive Tree-Aware Embedding model, semiDATE """
    ...
    def train_semiDATE_model(self):
        """ Train semiDATE model """
        
        print(f'Mode: {self.args.mode}, Episode: {self.data.episode}')
        
        if self.args.mode == 'scratch' or self.data.episode == 0:
            self.date_model = VanillaSemiDATE(self.data, self.args)
        else:
            self.date_model = self.get_model()
            self.date_model = VanillaSemiDATE(self.data, self.args, self.date_model.state_dict())
    ...
    def query(self, k, model_available = False):
        if not model_available:
            ...
            self.prepare_semiDATE_input()
            self.train_semiDATE_model()
        self.predict_frauds()
...
class VanillaSemiDATE:
    ...
    def evaluate(self):
        ...
        print()
        print("--------Evaluating semiDATE model---------")
        # create best model
        best_model = torch.load(self.model_path)
        best_model.eval()
