...
def uncertainty_tag(train, unc, option) :
    if option == 'naive' :
        ...
        xgb_illicit.fit(pd.DataFrame(train, columns=column_to_use_unc_measure), pd.DataFrame(train, columns=['illicit']).values.ravel())
        return unc.dot(xgb_illicit.feature_importances_ / sum(xgb_illicit.feature_importances_))
...
def uncertainty_measurement(train, valid, test, option) :
    ...
    res = uncertainty_tag(train, unc, option)
    return res
